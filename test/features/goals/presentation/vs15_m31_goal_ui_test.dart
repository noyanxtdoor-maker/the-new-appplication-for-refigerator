...
void main() {
  testWidgets('M3.1 Goal presentation keeps the canonical bar and same-ID save', (
    tester,
  ) async {
    ...
    // `scrolledUnderElevation: 0` and `goal-edit-blank-space-dismiss` exist
    // nowhere in lib/ - the canonical InternalAppBar/InternalScreen
    // architecture owns bar chrome and dismissal (see the M3.1 source-token
    // contract). Assert the canonical bar plus Edit Goal's real controls and
    // keep the same-ID save semantics this test exists to protect.
    expect(
      find.descendant(
    ...
    expect(find.byKey(const Key('goal-edit-back')), findsOneWidget);
