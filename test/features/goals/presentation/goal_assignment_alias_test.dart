...
void main() {
  ...
  testWidgets('edit: alias follows the unsaved draft and no nested editor exists',
      (tester) async {
    ...
    // The nested editor action is removed by contract G.
    expect(find.byKey(const Key('goal-edit-event-type')), findsNothing);
    ...
    await tester.enterText(find.byKey(const Key('goal-title')), 'Learn Spanish');
    await tester.pump();
    expect(
      find.descendant(
        of: find.byKey(const Key('goal-assigned-event-type')),
    ...
    for (final entry in beforeById.entries) {
      expect(afterById[entry.key], entry.value,
          reason: 'row ${entry.key} must be untouched by the Goal flow');
    }
    expect(
      typesAfter.where((row) => row.stableKey == 'job_application').single.label,
      'Job Application',
      reason: 'the canonical row keeps its seed label; Goal flows never rename it',
    );
  });
