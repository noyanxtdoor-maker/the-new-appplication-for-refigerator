...
//   3. Grip color follows the app theme family WITH CONTRAST: grip =
//      colorScheme.primary, fill = colorScheme.primaryContainer (existing
//      semantic tokens), so grip and fill are always distinguishable.
...
void main() {
  ...
  Future<void> runMatrixCell(
    ...
    // MP-06B: grip and fill must be visually distinguishable (contrast).
    final ratio = _contrastRatio(grip, fill);
    expect(
      ratio,
      greaterThanOrEqualTo(3.0),
      reason: 'MP-06B: grip ($grip) vs fill ($fill) must be distinguishable '
          'in $appearance/$themeColor ($eventTypeKey); got contrast $ratio',
    );
