...
void main() {
  group('G1 — dedicated notification small icon', () {
    ...
    test('approved VectorDrawable exists as a monochrome single-fill vector', () {
      ...
      expect(drawable, contains('android:viewportWidth="108"'));
      expect(drawable, contains('android:viewportHeight="108"'));
      ...
      // No baked square background: no full-viewport rectangle path data.
      expect(drawable, isNot(contains('M0 0L108 0L108 108L0 108')));
      // Derived from the approved hand + planner logo (traceable comment).
      expect(drawable, contains('06cfded9-7c0a-46c4-a9bf-ae4678fdaf1e.svg'));
    });

    test('gateway initializes the small icon from the dedicated drawable', () {
      expect(
        gatewaySource,
        contains("AndroidInitializationSettings('@drawable/ic_nt_notification')"),
      );
  ...
  group('G2 — Event detailed notification content', () {
    test('display title falls back to the Event Type label when title blank', () {
    ...
    test('reminder body keeps From-To and appends non-empty description only', () {
      // Body law: "<From>–<To>\n<description if non-empty>".
      final occurrence = _occurrence(
        notes: 'Prepare chapter 4 notes',
      );
    ...
    test('content refresh keeps one stable reminder identity per occurrence', () {
  ...
  group('G5 — dynamic inherited reminder wording', () {
    ...
    test('policy modes still hydrate Off/Custom per item without mutation', () {
      ...
      final off = policy.copyWith(mode: ReminderPolicyMode.off, clearOffset: true);
