...
final class DriftPlannerRepository
    implements
        PlannerRepository,
        ...
        await (database.select(database.plannerTasks)
              ..where(
                (table) =>
                    ...
                    table.dueDate.isBiggerOrEqualValue(startDate.iso8601) &
  ...
  Future<List<String>> readActionableBadgeTasks({
    ...
    if (endDate.compareTo(startDate) < 0) {
      throw ArgumentError('Badge Task range must not end before it starts.');
    }
    final rows =
        await (database.selectOnly(database.plannerTasks)
              ..addColumns(<Expression<Object>>[database.plannerTasks.id])
              ..where(
                database.plannerTasks.profileId.equals(profileId) &
                    database.plannerTasks.status.equals(
                      PlannerTaskStatus.incomplete.name,
                    ) &
                    database.plannerTasks.dueDate.isNotNull() &
                    database.plannerTasks.dueDate.isBiggerOrEqualValue(
                      startDate.iso8601,
                    ) &
                    database.plannerTasks.dueDate.isSmallerThanValue(
                      endDate.addDays(1).iso8601,
                    ),
              )
              ..orderBy(<OrderingTerm>[
                OrderingTerm.asc(database.plannerTasks.dueDate),
                OrderingTerm.asc(database.plannerTasks.id),
              ]))
            .get();
    return rows
        .map((row) => row.read(database.plannerTasks.id)!)
        .toList(growable: false);
  }
  ...
  Future<Map<String, OutcomeKind>> _readEffectiveTaskOutcomes({
    ...
    return <String, OutcomeKind>{
      for (final row in rows)
        row.sourceId: OutcomeKind.values.byName(row.outcome!),
    };
  }
